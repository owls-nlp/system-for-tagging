...
class SSD(nn.Module):
    ...
    def __init__(self, cfg, base, extras, head):
        ...
        if self.phase == 'test':
            ...
            self.detect = Detect(cfg, self.num_classes, 0, 200, 0.01, 0.45)
...
def build_ssd(cfg, size=512, num_classes=2):

    base_, extras_, head_ = multibox(vgg(base, 3, False),
                                     add_extras(cfg, size, 1024),
                                     cfg['mbox'][str(size)], size, num_classes)

    return SSD(cfg, base_, extras_, head_)
